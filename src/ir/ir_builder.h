...
class IRBuilder final : ast::ASTVisitor {
    ...
    BlockID new_block() {
        auto id = BlockID(current_func->blocks.size());
        current_func->blocks.emplace_back(id);
        return id;
    }

    void switch_block(BlockID block) { current_block = block; }
    ...
    VReg emit_inst(IROp op, type::TypeRef dest_type,
    ...
    */
    ...
    void add_phi_operand(InstId phi, VReg var, BlockID block_id) {
        ...
        inst.operands.push_back(Operand::label(block_id.id));
    }
    ...
    VReg try_remove_trivial_phi(InstId inst_id, BasicBlock& block) {
        ...
        for (auto& [user_inst, user_block] :
            ...
            for (auto& reg : get_inst(user_inst).operands) {
                if (reg.is_reg() && reg.as_reg().id == phi.dest.id) {
